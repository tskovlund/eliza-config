...
1. Edit in your persistent clone: `/var/lib/zeroclaw/repos/eliza-config`
2. Commit and push (see Git Identity below)
...
- Commits sign automatically → ✅ Verified badge on GitHub

### Shell policy: plain commands only

`git` works fine in `shell` — but the security policy blocks shell operators and subshells. Rules:

- ✅ `git status` — plain commands work
- ✅ `git add .` then `git commit -m "message"` — separate calls, no chaining
- ❌ `git status 2>&1` — redirects blocked
- ❌ `$(git rev-parse HEAD)` — subshell substitution blocked
- ❌ `git add . && git commit` — `&&` chaining blocked

If you need stderr or complex scripting, use Python subprocess:

```python
import subprocess, os
result = subprocess.run(
    ["git", "-C", "/var/lib/zeroclaw/repos/eliza-config", "status"],
    capture_output=True, text=True
)
print(result.stdout, result.stderr)
```

The built-in `git_operations` tool also works but operates on the current working directory only — not the persistent clone.
